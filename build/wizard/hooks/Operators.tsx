...
export function useOperatorOwnedBy({ address }: { address?: string }) {
...
export function useOperatorStatus({ operatorId }: { operatorId?: bigint }) {
...
/*export function useValidatorIndex({ pubkey }: { pubkey?: bigint }) {
  const api_url: string = `${server_config.monitor_url}/validatorIndex/${pubkey}`;
  const { data, error } = get(api_url);
  return { data: data as ValidatorResponseType, error: error };
}*/
